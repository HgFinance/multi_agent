...
부서 실행 계층은 모든 부서에서 동일하다. Hermes Agent가 Codex 또는 Claude Code에 연결된 부서장이며, 부서 직원은 역할별 독립 LangGraph Worker Graph와 Ollama `qwen3:1.7b`를 사용한다. Worker는 허용된 도구 결과를 `worker-context.v1`로 만들어 부서장에게 전달할 뿐이며, Risk/QA 결정론 엔진의 바인딩 판정이나 CEO의 최종 비바인딩 종합을 대체하지 않는다.
