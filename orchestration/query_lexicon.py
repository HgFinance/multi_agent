...
# `안`·`못`은 한 음절이라 두 방향으로 틀린다.
#
# 1. 다른 낱말 **안에** 들어간다. `"5거래일 동안 추적"`의 `동안 `이 부정으로
#    잡혀 정상 트레일링 조건주문이 막혔다(2026-08-31 실측). 앞이 한글이면
#    어절이 아니므로 표지로 보지 않는다.
# 2. 뒤 용언에 **붙여 쓴다.** 공백을 요구하면 `"주문 안하고"`·`"매도 못하게"`
#    처럼 띄어쓰기를 생략한 부정을 통째로 놓친다. 그래서 뒤에 오는 것이
#    공백이거나 용언 어간일 때를 함께 본다 - `안전`·`안내`·`안정`처럼
#    부정이 아닌 낱말은 이 어간 목록에 없어서 걸리지 않는다.
_STANDALONE = r"(?<![가-힣])"
_NEGATION_STEM = r"(?=\s|하|되|된|할|함|해|했|사|살|팔)"
_NOT_ABLE_STEM = r"(?=\s|하|되|된|할|함|해|했)"
...
_SHARED_NEGATION_PATTERN = (
    ...
    r"수행하지\s*(?:마|말라|마세요|말)|"
    rf"{_STANDALONE}안{_NEGATION_STEM}|{_STANDALONE}못{_NOT_ABLE_STEM}|"
    r"금지|불가|없"
)
