...
2. `CSPAQ12300`의 BEP 단가와 `t0424`의 평균단가는 의미가 다르므로 별도 표시한다. 수량 대사는 `CSPAQ12300.BnsBaseBalQty`(매매기준 잔고수량)와 `t0424.janqty`(체결기준 잔고수량)를 비교하고, `BalQty`(D+2 잔고수량)는 별도 보존한다.
