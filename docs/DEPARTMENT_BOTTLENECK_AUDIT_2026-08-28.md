...
| 의존성 lock/SBOM/CVE | NOT VERIFIED | requirements 직접 의존성은 보강했지만 Python lockfile/SBOM/CVE 결과 없음 |
...
- `.venv/bin/pytest -q`: **3594 passed, 31 skipped, 232 subtests passed** (159.54초, 경고 2건은 외부 라이브러리 deprecation).
...
3. HR 비용 측정은 token이 실제로 관측된 경우에만 기록하고, 미측정은 `UNKNOWN`으로 유지한다. local boundary latency를 cost 대용으로 쓰지 않는다.
4. 부서별 Notion/Discord report에는 `department`, `run_id`, `source`, `observed_at`, `latency_ms`를 필수로 묶어 공통 CEO 채널의 혼합 로그를 부서별 수치처럼 집계하지 않는다.
