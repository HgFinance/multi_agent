...
전체 61거래일을 날짜별로 다시 점검한 결과, 한국시간 2026-08-12에만 비정상적인
호가 결손이 발견됐다. 2,494종목 중 917종목(36.7682%)은 체결이 있었지만 호가가
0건이었다. 인접 거래일의 같은 유형은 1종목 수준이므로 조용한 종목의 자연스러운
결측으로 볼 수 없다. 기존 `feed_gaps`는 원천 전체가 0건인 날만 감지해 이 사건을
기록하지 못했다.

빌더는 이제 반대쪽 원천이 살아 있는 종목 중 10% 이상이면서 최소 50종목에서 한쪽
원천이 0건이면 `partial_universe_loss`를 `HIGH/OPEN` 사건으로 기록한다. 재실행 중복은
막고, 진단 INSERT가 실패해도 savepoint로 피처 적재 트랜잭션은 보존한다. 과거 사건도
`market.feed_gaps`에 소급 기록했다. 해당 날짜의 호가 기반 피처는 알파 검증에서 제외하거나
원천 복구 후 v4 데이터셋을 새 content hash로 다시 만들어야 한다.
