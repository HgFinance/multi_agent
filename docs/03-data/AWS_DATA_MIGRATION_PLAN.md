...
## 4.1 Parquet 이 재계산을 막지 않으려면 (실측 2026-08-12)

Parquet 은 zip 같은 압축이 아니라 **컬럼 저장 + 무손실 인코딩**이다. 같은 값이
그대로 나오고 `NULL` 도 `NULL` 로 보존된다("못 잰 것 ≠ 0" 규칙이 살아 있다).

**그래서 형식이 문제가 아니라 내보내는 방식이 문제다.** 셋을 지킨다.

### ① 시각은 마이크로초로 쓴다

실측(8/12 09:00~09:10, 체결 1,169,325건):

| | 마이크로초 있음 |
|---|---|
| `event_time` | 1,148,497 (98%) |
| `received_at` | 1,169,325 (100%) |

거의 전부가 초 미만 정밀도를 갖는다. Parquet 타임스탬프를 밀리초로 쓰면
**같은 밀리초 안의 순서가 사라진다** — 마이크로구조는 체결 순서와 호가 갱신
간격이 곧 신호다. `TIMESTAMP(MICROS)` 이상.

### ② `numeric` 을 `double` 로 캐스팅하지 않는다

`best_bid`·`mid_price`·`spread` 는 `numeric`(정확한 십진)이다. `DOUBLE` 로
내보내면 반올림이 들어가고, 스프레드를 bps 로 환산할 때 누적된다. `DECIMAL` 로
쓴다.

### ③ **원시 내보내기는 피처가 아니라 행 전체다**

이게 제일 크다. Parquet 이 손실을 내는 것이 아니라, *"지금 피처가 쓰는 열만"*
내보내는 순간 v2 정의가 막힌다.

`ms-daily-v1` 이 쓰는 것은 `spread`·`depth_imbalance`·`side`·`quantity`·`price`
뿐이다. 그런데 원본에는 `bid_prices[10]`·`bid_sizes[10]`·`ask_prices[10]`·
`ask_sizes[10]`·`total_bid_size`·`trading_status`·`session_type` 이 다 있다.
v2 에서 호가 5단계 깊이 기울기나 동시호가 구간 분리를 쓰려면 그 열들이 있어야
한다.

> 재계산을 위해 남기는 데이터에서 **지금 안 쓰는 열을 빼면, 그건 재계산을 위한
> 데이터가 아니다.** 원시 파티션은 `select *` 다.
...
- **못 옮긴 것을 옮긴 것으로 세지 않는다.** `content_hash` 가 같아야 같은 것이다.
  해시는 **읽어들인 값**에서 계산한다 - 내보낼 때 계산하고 넣을 때 안 다시 재면
  형식 변환에서 값이 바뀌어도 통과한다(§4.1 의 캐스팅·정밀도가 바로 그 지점이다).
