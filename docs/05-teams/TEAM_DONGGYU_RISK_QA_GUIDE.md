...
> 실행 계층 확정(2026-08-03): 부서장은 Hermes + Codex/Claude Code, 직원은 독립 LangGraph Worker + Ollama `qwen3:1.7b`다. Risk는 4개 Worker, QA는 5개 Worker로 통합했으며, 중복된 기존 RSK/QAA 역할명은 Profile·감사 이력 호환 Alias로만 유지한다. 상세 매핑은 [Department Worker Graph Architecture](../02-engineering/DEPARTMENT_WORKER_GRAPH_ARCHITECTURE.md)를 기준으로 한다.
...
- 동규님은 Kanban `agent.status.v1` 매핑, Runtime Health 우선순위, 권한 분리와 상태 오표시 방지 Test의 독립 Reviewer다. 공통 Frontend 구현은 도현님, Live Office Business Ownership은 영주님이 맡는다.
...
- Kanban의 `WAITING_APPROVAL`·`BLOCKED` 표시는 업무 진행 상태다. 실제 Risk·QA 승인 여부는 `risk.*`·`qa.*` 공식 Event로만 판정한다.
