...
되지만 DB 왕복이 늘어난다). ⚠ 거래소→수신 구간은 정밀 측정이 안 된다 —
LS가 체결 시각을 초 단위로만 주고, **개발 PC 시계가 표준시보다 약 1.46초
느린 것을 발견**(Supabase now() 교차 측정)해 절대 시각에 계통 오차가 있다.
w32time 서비스가 꺼져 있어 관리자 권한으로 `Start-Service w32time; w32tm
/resync` 실행이 필요하다(재일님 조치).
