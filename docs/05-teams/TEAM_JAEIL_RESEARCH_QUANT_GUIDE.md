...
- SearXNG 기반 Web Search, Evidence 승격 규칙, 데이터 품질 Gate와 분석가 Tool Runner를 파이프라인에 연결했다.
- `Insight`를 1급 객체로 만들고 Evidence Gap 재탐색, 심의 순환, 방법 성과 학습과 업종별 기준선을 추가했다.
- 기술·재무·미시구조 분석을 보강하고 KSIC 업종 문맥과 Piotroski·Altman 지표가 실제 분석가 출력에 도달하도록 배선했다.
- `ResearchCaseV2`, `AnalystFindingV1`, `ResearchPacketV2`, `HypothesisSpecV2`, `ExperimentCardV1` 계약을 구현했다.
- `QNT-08` Investment Doctrine Model Factory의 Prompt/RAG Baseline, Fine-tuning Need Gate와 독립 Frozen Eval 설계를 확정했다.
- 일봉 신선도 API, 거래일 기준 Staleness Gate, 15:50 일봉 증분 Job과 장중 현재가 보강을 `4788943`으로 반영했다.
- 위 변경은 `4788943`까지 `main`에 병합됐으며 리서치·퀀트 원격 작업 브랜치는 `main`보다 앞선 Commit이 없다.

### Today

- [ ] `RQ-01`: 실제 종목 한 건으로 `ResearchPacketV2`를 생성해 `artifact_id`, `case_id`, `trace_id`,
  `as_known_at`, Claim·Evidence·Insight 연결과 PIT Manifest가 모두 채워지는지 검증한다.
- [ ] 검증된 Packet JSON을 고정 Fixture로 저장하고 도현님에게 OrderIntent 입력 계약, 동규님에게
  Risk·QA Evidence 입력 계약으로 인계한다.
- [ ] `4788943`의 일봉 신선도 Gate, 증분 Job과 장중 현재가 경로를 깨끗한 Docker 환경에서 재실행하고 결과를 남긴다.
- [ ] 7월 31일 이후 누락된 일봉을 백필하고 `last_bar_date`, 대상 종목 수와 누락 거래일 수를 실행 증거로 남긴다.
- [ ] `QNT-08`: 먼저 Prompt/RAG Baseline과 Frozen Eval Fixture를 구현할 Task를 쪼개고, Baseline을 이기기 전에는
  유명 투자자 Persona Fine-tuning을 시작하지 않는 Gate를 체크리스트로 고정한다.
- [ ] `RQ-05`: `microstructure_features` 0건 문제의 최소 Producer 입력·출력과 Replay Hash 계약을 확정한다.

### Blocker

- `CI-06` Migration 목록 누락은 `main`에서 수정됐지만 최신 `main` 기준 전체 pytest 통과 로그는 아직 확인되지 않았다.
- 일봉 신선도 Gate와 증분 수집 Job은 병합됐지만 7월 31일 이후 실제 백필과 최신 봉 날짜 검증은 아직 남아 있다.
- `RQ-01`의 전사 Event Envelope은 도현님 `PLAT-01`, Project Redis와 Outbox는 `PLAT-02`가 선행한다.
- Quant Strategy 승격은 동규님의 QA/Risk Gate와 영주님의 Case·Approval ID 없이는 완료 처리하지 않는다.
- 미래 거래일 Calendar와 지수 구성 이력의 승인 Source는 아직 확정되지 않았다.

### 2주 개인 실행 계획

| 순서 | 기간 | Task | 산출물 | 선행 조건 | 완료·인계 기준 |
|---|---|---|---|---|---|
| 1 | 완료 | `CI-06` | Migration 기대 목록과 Schema Test | 없음 | `main` 수정 완료, 전체 Suite 재검증 대기 |
| 2 | 08-04~05 | `RQ-01` | `ResearchPacketV2` Schema·Fixture·Artifact ID | 도현 `PLAT-01` Review | 동규 QA, 도현 Trading이 같은 Fixture 소비 |
...
과거 성능 비교에서는 로컬 Ollama `qwen3:14b`를 사용했다. 아래의 모델 선정·후보·예비 등재 표는 당시 실험 결과인 Historical snapshot이며 현재 배정이 아니다. 2026-08-03 확정된 현재 임시 테스트 Worker 기본값은 독립 LangGraph Worker + `qwen3:1.7b`로 통일한다.
비용 0, 전부 실측 검증:
...
- 과거 qwen3:8b(5.2GB)는 이전 기준의 후보로만 남긴다 —
