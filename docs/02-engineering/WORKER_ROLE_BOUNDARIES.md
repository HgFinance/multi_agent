...
- **Quant**: `strategy-author-worker`, `result-interpretation-worker`. 전략 코드 작성과 결과 해석만 LLM Worker가 맡고, PIT Dataset·Backtest·Release·비용·Regime 계산과 판정은 결정론 `pipeline/`이 소유한다.
