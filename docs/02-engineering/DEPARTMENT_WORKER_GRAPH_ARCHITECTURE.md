...
- 직원: 역할별 독립 LangGraph Worker Graph + 운영 Qwen AWQ v1(`qwen2.5-14b-instruct-awq`); 로컬 Ollama `qwen3:1.7b`는 개발 fallback
...
Worker별 모델 선택 규칙은 [WORKER_MODEL_MATRIX.md](WORKER_MODEL_MATRIX.md)에 고정한다. 현재 운영 기본값은 모든 LLM Worker의 Qwen AWQ v1이며, 모델 호출은 Worker Model Gateway가 담당한다. arithmetic LoRA는 명시적 route가 없는 한 base model로 남고, FinanceBench HOLD가 해소되기 전 품질 승격으로 해석하지 않는다.
...
| Employee Worker | LangGraph + Worker Model Gateway → Qwen AWQ v1 | 허용된 도구 호출, 근거 요약, 역할별 context 생성 | 바인딩 승인, 임의 API/도구 호출 |
