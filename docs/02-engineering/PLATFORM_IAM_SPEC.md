...
- [ ] Role 비밀번호는 SecretManager (코드 하드코딩 금지) — 현재 `service.py`의 `role_password_provider` 기본값은 `os.urandom(16).hex()`로 매번 새로 생성만 함, SecretManager 저장·조회 연동 없음(비밀번호를 어디에도 기록하지 않아 조회가 애초에 불가능 — 이 자체가 후속 과제)
...
- [ ] Role 비밀번호 저장/조회 (SecretManager) — 현재 매번 새로 생성해 아무 데도 안 남기므로, Role은 만들어지지만 그 비밀번호로 실제 로그인할 방법이 없다. Agent 런타임이 이 Role로 실제 접속하려면 이 저장 경로가 먼저 필요하다
