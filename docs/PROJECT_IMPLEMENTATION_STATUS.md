...
> **Risk/QA 보안 보정(2026-08-05)**: Worker Scope fail-closed, QA runtime 미설정 DENY, Risk/QA 명령 인증은 `IMPLEMENTED`/`TEST_VERIFIED`다. Redis Event Bus와 rollback형 DB/Event smoke, Risk/QA Compose 기동·formal healthcheck는 `RUNTIME_VERIFIED`다. 실제 Research Packet Replay와 전역 Issuer·mTLS·IAM 매핑은 여전히 `BLOCKED`다.
