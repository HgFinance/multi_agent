...
> **Risk/QA 보안 보정(2026-08-05)**: Worker Scope fail-closed, QA runtime 미설정 DENY, Risk/QA 명령 인증은 `IMPLEMENTED`/`TEST_VERIFIED`다. Risk↔QA Redis Event Bus는 외부 통합 테스트 `11 passed`로 `RUNTIME_VERIFIED`가 됐고, Docker/DB Replay와 전역 Issuer·mTLS·IAM 매핑은 여전히 `BLOCKED`다.
