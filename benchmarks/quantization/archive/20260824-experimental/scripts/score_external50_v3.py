...
def numeric_equivalent(pred, gold, question=""):
    """
    ...
    Decimal <-> percent scaling is allowed only when percentage/rate
    semantics are explicit in the question or prediction.
    """
    ...
    semantic_text = f"{question} {pred}".lower()

    allow_percent_scale = bool(
        re.search(
            r"percentage|percent|portion|growth rate|rate of "
            r"|rate\?|ratio expressed as|%",
            semantic_text,
            flags=re.IGNORECASE,
        )
    )
...
def main():
    ...
    output = {
        "scorer": "HgFinance-External50-Scorer-v3-final",
        "raw_file": args.raw,
        "model": raw["model"],
    ...
    print("=" * 72)
    print("HgFinance External-50 Rescore v3-final")
    print("=" * 72)
