...
export type LangsmithQaTraces = {
  ...
  cache_reason?: "ttl" | "rate_limit" | "inflight" | "error" | null;
};

export function explainLangsmithError(body: unknown, status: number): string {
  if (typeof body === "object" && body !== null && "detail" in body) {
    const detail = (body as { detail?: unknown }).detail;
    if (typeof detail === "string" && detail.trim()) return detail;
    if (detail && typeof detail === "object") {
      const value = detail as { error_code?: unknown; message?: unknown };
      if (typeof value.message === "string" && value.message.trim()) return value.message;
      if (typeof value.error_code === "string" && value.error_code.trim()) return value.error_code;
    }
  }
  return `LangSmith 요청 실패 (HTTP ${status})`;
}
...
export async function fetchQaLangsmithTraces(days = 7): Promise<LangsmithQaTraces> {
  ...
  if (!response.ok) throw new Error(explainLangsmithError(body, response.status));
