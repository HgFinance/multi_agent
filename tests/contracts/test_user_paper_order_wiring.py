...
def test_local_bff_has_private_trading_transport_and_payload_proof() -> None:
    ...
    assert not any(key.startswith("LS_ACCOUNT") for key in bff)


def test_eb_wires_same_proof_to_bff_and_strict_paper_trading_api() -> None:
    ...
    assert not any("LIVE" in key or key.startswith("LS_ACCOUNT") for key in bff)
