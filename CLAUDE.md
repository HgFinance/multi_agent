...
- **현재 Worker Registry**: CEO 1, HR 0(직원 LLM 계층 없음 — 부서장 + 결정론 함수), Research 6, Trading 6, Risk 4, Quant/Backtest 7, Accounting/Portfolio 8, AI QA 5. 실제 런타임 수는 `workers`와 `runtime_personalities`를 기준으로 하며 `agent.personalities`의 기존 ID는 호환·감사 Alias로만 유지한다.
