...
- **현재 Worker Registry**: LLM 기준 CEO 1, HR 5, Research 6, Trading 2, Risk 1, Quant/Backtest 7, Accounting/Portfolio 1, AI QA 2이며 결정론 runner는 `desk-runner`, `risk-runner`, `qa-runner`, `back-office-runner` 각 1개다. 총 직원 수는 29명이다. 실제 런타임 수는 `workers`와 `runtime_personalities`를 기준으로 하며 `agent.personalities`의 기존 ID는 호환·감사 Alias로만 유지한다.
