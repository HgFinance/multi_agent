...
Local Model은 [`Modelfile`](Modelfile)의 `qwen3:14b` 기반 `agent-accounting`이며, Build·Eval·권한 기준은 [Ollama Department Modelfile Guide](../../docs/02-engineering/OLLAMA_DEPARTMENT_MODELFILE_GUIDE.md)를 따른다. 가이드 2절 표는 초기 등록값(`qwen2.5` / `accounting-portfolio-department`)이라 실물과 다르다 — 팀 공용 호출명은 `agent-accounting`이다(`departments/06-ai-qa-audit/scripts.py`).
