...
class Artifact:
    ...
    artifact_type: str  # "research_packet" | "order_intent" | ...
    producer: str
    fund_id: UUID
    trace_id: UUID
    claims: tuple[Claim, ...]
...
class EvidenceQaEngine:
    ...
    def check_artifact(
        ...
        if not artifact.claims:
            raise QaEngineError("Claim이 하나도 없는 Artifact는 검사할 수 없습니다")
    ...
    def _check_claim(
        ...
        if not claim.text.strip():
            return (
                ClaimCheckResult.UNSUPPORTED, "Claim 텍스트가 비어 있습니다",
                CheckFailureReason.MISSING_REQUIRED_FIELD,
            )
        if claim.kind is ClaimKind.FACT and not claim.subject:
            return (
                ClaimCheckResult.UNSUPPORTED, "Fact Claim에 주체(subject)가 없습니다",
                CheckFailureReason.MISSING_REQUIRED_FIELD,
            )
...
if __name__ == "__main__":
    ...
    a1 = artifact_with(Claim(0, "AAPL 종가는 70000원", ClaimKind.FACT, subject="AAPL",
                             numeric_value=Decimal("70000"), unit="KRW", evidence_ids=(ev1.evidence_id,)))
    ...
    a2 = artifact_with(Claim(0, "AAPL은 반등한다", ClaimKind.FACT, subject="AAPL"))
    ...
    a3 = artifact_with(Claim(0, "다음 주 반등 가능성이 있다", ClaimKind.INFERENCE))
    ...
    a4 = artifact_with(Claim(0, "매출이 늘었다", ClaimKind.FACT, subject="매출",
                             evidence_ids=(uuid4(),)))
    ...
    ev5 = evidence()
    a5 = artifact_with(Claim(0, "내부 보고서에 따르면", ClaimKind.FACT, subject="내부",
    ...
    a6 = artifact_with(Claim(0, "내일 발표 예정", ClaimKind.FACT, subject="발표",
    ...
    a7 = artifact_with(Claim(0, "AAPL 종가는 70000원", ClaimKind.FACT, subject="AAPL",
                             numeric_value=Decimal("70000"), unit="KRW", evidence_ids=(ev7.evidence_id,)))
    ...
    a8 = artifact_with(Claim(0, "AAPL 종가는 약 6만원대", ClaimKind.FACT, subject="AAPL",
    ...
    a9 = artifact_with(Claim(0, "AAPL 종가는 출처마다 다르게 보고됨", ClaimKind.FACT, subject="AAPL",
                             evidence_ids=(ev8a.evidence_id, ev8b.evidence_id),
    ...
    a10 = artifact_with(
        Claim(0, "AAPL 보유량은 100주", ClaimKind.FACT, subject="AAPL",
    ...
    a12 = artifact_with(Claim(0, "AAPL 종가는 70000원", ClaimKind.FACT, subject="AAPL",
    ...
    a13 = artifact_with(
        Claim(0, "AAPL 종가는 70000원", ClaimKind.FACT, subject="AAPL",
              numeric_value=Decimal("70000"), unit="KRW", evidence_ids=(ev13.evidence_id,)),
        Claim(1, "MSFT는 반등 여력이 있다", ClaimKind.INFERENCE),
        Claim(2, "삼성전자 실적이 개선됐다", ClaimKind.FACT, subject="삼성전자"),  # 근거 없음
    )
    ...
    # 14. Claim 텍스트가 비어있음 -> UNSUPPORTED (Schema 위반)
    a14 = artifact_with(Claim(0, "   ", ClaimKind.INFERENCE))
    r14 = engine.check_artifact(a14, ctx_with(EvidenceStore()))
    result_is(r14, 0, ClaimCheckResult.UNSUPPORTED, "빈 Claim 텍스트")
    assert CheckFailureReason.MISSING_REQUIRED_FIELD in r14.reason_codes

    # 15. Claim이 하나도 없으면 검사 자체를 거부한다 (조용히 통과시키지 않음)
    try:
        engine.check_artifact(artifact_with(), ctx_with(EvidenceStore()))
        raise AssertionError("Claim 없는 Artifact를 통과시켰다")
    except QaEngineError:
        pass

    # 16. 재현성 - 같은 Artifact·Context를 두 번 평가하면 같은 판정
    r16a = engine.check_artifact(a1, ctx_with(store_with(ev1)))
    r16b = engine.check_artifact(a1, ctx_with(store_with(ev1)))
    assert r16a.decision is r16b.decision
    assert [c.result for c in r16a.claim_checks] == [c.result for c in r16b.claim_checks]

    print("ok - Evidence QA Engine 16개 시나리오 점검 통과")
