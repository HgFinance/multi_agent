...
class IncidentTimeline:
    ...
    def add_event(
        ...
        *,
        ...
        event = IncidentEventRecord(
            incident_event_id=uuid4(), incident_id=incident_id, source=source,
    ...
    def open_corrective_action(
        ...
        *,
        ...
        if incident_id is None and finding_id is None:
            raise IncidentTimelineError("Corrective Action은 Incident나 Finding 중 하나에는 연결돼야 합니다")
    ...
    def verify_and_close(
        self, corrective_action_id: UUID, verifier: str, verification: dict,
    ) -> CorrectiveActionRecord:
        """QA 검증 후에만 Close한다(팀 가이드 6.3 불변식) - verifier 없이 COMPLETED로
        못 간다. 조치를 만든 사람이 스스로 검증자가 될 수도 없다."""
        action = self._require_action(corrective_action_id)
        if action.status not in (CorrectiveActionStatus.IN_PROGRESS, CorrectiveActionStatus.VERIFYING):
            raise IncidentTimelineError(
                f"상태 {action.status.value}에서는 검증·종료할 수 없습니다 (IN_PROGRESS/VERIFYING만 가능)"
            )
        if verifier == action.owner:
            raise IncidentTimelineError("조치를 실행한 담당자가 스스로 검증할 수 없습니다")
    ...
    def _require_action(self, corrective_action_id: UUID) -> CorrectiveActionRecord:
        ...
        if action is None:
            raise IncidentTimelineError(f"존재하지 않는 Corrective Action입니다: {corrective_action_id}")
        return action
