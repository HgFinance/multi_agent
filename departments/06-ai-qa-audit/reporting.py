...
def langsmith_handoff(trace_id: str) -> dict[str, Any]:
    enabled = os.environ.get("LANGCHAIN_TRACING_V2", os.environ.get("LANGSMITH_TRACING", ""))
...
def evaluation_metrics(out: dict[str, Any], report_markdown: str = "") -> dict[str, Any]:
    ...
    unsupported = sum(1 for claim in claims if claim.get("result") in {"UNSUPPORTED", "CONTRADICTED"})
