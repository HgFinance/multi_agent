...
_RULES: tuple[tuple[re.Pattern, str, str], ...] = (
    ...
    # ▶ **실행면이 이미 "못 한다"고 이름까지 적어 놓은 경우다** (2026-08-14)
    #   `config_binding.bind` 는 `NOT_IMPLEMENTED` 에 사유가 있으면
    #   "미구현: <사유>" 로 거부한다. 같은 갈래의 다른 변형("어휘에 없다")은
    #   위 규칙이 이미 잡으므로 이것을 **그 뒤에** 둔다 - 앞에 두면 템플릿을
    #   더하라는 더 나은 지시를 가로챈다.
    (re.compile(r"미구현:|통제 어휘로 사상할 수 없다"),
     IMPLEMENTATION,
     "실행면이 그 엣지를 아직 못 만든다고 사유까지 적어 뒀다. 그 사유를 "
     "없애 구현하든지, 리서치에 다른 엣지로 다시 내라고 돌려줘라 - "
    ...
    # ── 가설: 기획안이 계약과 안 맞는다 ──────────────────────────────────────
    # ▶ 이 무리 전체가 **`불명` 이 만들어 준 것이다** (2026-08-14, 카드
    #   t_ad5f27e1 #2). 실행면(`config_binding.bind`)의 거부 갈래 10개를 실제로
    #   굴려 판정자에 먹여 봤더니 **18개 표면이 전부 `불명`** 이었다. 규칙은
    #   2026-08-12 의 발신 문장을 보고 적었는데, 그 뒤 실행면이 값 검사·어휘
    #   검사·짝 검사·수식 검사를 새로 얻는 동안 여기는 안 따라왔다.
    #
    #   더 나쁜 것은 **자체점검이 초록이었다**는 점이다 - 픽스처가
    #   "실행면이 안 읽는 파라미터: universe" 라는, 발신자가 한 번도 낸 적
    #   없는 문장이었다(soundness·census·autopilot 세 곳 모두). 선언과 실재가
    #   갈린 전형이라 `factory_autopilot` 쪽에 **발신자를 실제로 부르는**
    #   점검을 같이 남긴다 - 규칙만 고치면 다음 갈래에서 또 뚫린다.
    #
    #   수준은 전부 `가설` 이다. 값·형·어휘·짝·수식은 러너가 못 고친다 -
    #   자르거나 채우면 등록한 가설과 실행한 실험이 달라지고(실행면이 바로
    #   그래서 거부한다), 다시 등록할 수 있는 쪽은 리서치뿐이다.
    (re.compile(r"허용 범위 \[.*?\] 밖이다"),
     HYPOTHESIS,
     "기획안이 실행면의 허용 범위 밖 값을 썼다. **부호도 범위의 일부다** - "
     "낙폭 정지(`max_drawdown_stop`)는 음수여야 한다. 실행면은 자르지 "
     "않으므로(자르면 등록한 것과 실행한 것이 달라진다) 사유에 적힌 범위 "
     "안의 값으로 다시 등록해라"),
    (re.compile(r"(정수|수)로 읽을 수 없다"),
     HYPOTHESIS,
     "기획안이 수 자리에 수가 아닌 값을 썼다. 사유에 적힌 키를 수로 고쳐 "
     "다시 내라"),
    (re.compile(r"통제 어휘 밖이다"),
     HYPOTHESIS,
     "기획안이 실행면 통제 어휘 밖의 값을 썼다. 사유가 사용 가능한 값을 "
     "전부 적어 뒀으니 그 중 하나로 다시 내라 - 비슷한 것으로 대신 "
     "돌리지 않는다"),
    (re.compile(r"EXCLUDE_BOTTOM 일 때만|하위 몇 %를 빼는지는"),
     HYPOTHESIS,
     "구성 방식과 비율은 짝이다. 한쪽만 선언하면 실행면이 나머지를 "
     "기본값으로 채우게 되는데, 그 값은 아무도 선언한 적이 없다. 둘을 "
     "같이 적어 다시 내라"),
    (re.compile(r"알파 수식이 성립하지 않는다"),
     HYPOTHESIS,
     "기획안의 알파 수식이 파서를 통과하지 못한다. 사유에 적힌 자리를 "
     "고쳐 다시 내라 - 실행면은 반쪽짜리 수식으로 실험을 만들지 않는다"),
    # ▶ **표면이 두 가지다** (2026-08-14). 발신자(`config_binding`)는 "실행면이
    #   **읽지 않는** 파라미터가 있다" 로 내고, 오케스트레이터만 그 앞에
    #   "가설 파라미터 거부: " 를 붙인다. 규칙은 후자만 잡고 있어서, 건전성
    #   census 처럼 발신자 원문을 그대로 보는 경로는 전부 `불명` 이었다.
    (re.compile(r"(안 읽는|읽지 않는) 파라미터|파라미터 거부"),
     HYPOTHESIS,
