...
▶ **이 도구는 끝난 일회성 이관이다** (2026-08-11 종료, 재일님 결정)
  2026-05-18~08-11 73거래일을 가져오는 것으로 역할이 끝났다. 그 뒤로는
  우리 수집기(ls_realtime_service)가 통합시세 US3/UH1 로 KRX+NXT 를 직접
  받는다 - Trading_bot 은 **다른 프로젝트이고 운영 의존이 아니다.**

  일일 이관을 걸었다가 바로 걷어냈다. 같은 날을 두 경로로 받으면 한 날에
  두 출처가 섞여 무엇을 근거로 쓴 실험인지 흐려진다(실제로 8/11 에 그렇게
  됐고 지우고 다시 받아야 했다). **이 파일을 스케줄에 다시 걸지 마라.**

  그래도 지우지 않는 이유: market.pit_provenance 가 이 경로를 출처로
  가리키고 있다. 그 구간이 어디서 왔고 왜 PIT 가 없는지를 설명하는 유일한
  기록이다. 과거 구멍을 메울 때만 손으로 --redo 를 준다.
