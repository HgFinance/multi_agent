...
2. Select one information-gaining next step and preregister an experiment plan as JSON under {self.lab_root}/plans before measuring its result. Unless the user explicitly fixes an end date, determine the latest date/bar actually returned by the permitted data source at run time and include it in the experiment; never reuse a stale prior end date. Freeze and record `data_as_of`, `requested_end`, `actual_last_returned_bar`, and any source retention boundary in the plan/result. The plan must contain plan_id, hypothesis_id, objective, method, data_requirements, splits (development, validation, out-of-sample and forward-or-paper observation unless a concrete limitation is recorded), cost_model, seed (JSON key exactly `seed`, integer value; do not use `integer_seed`), signature, and a non-empty preregistration_hash.
