...
def _context_hash(intent: OrderIntent, ctx: RiskContext) -> str:
    ...
    payload = {
        ...
        "restricted_items": sorted(
            f"{r.instrument_id}:{r.restriction_type.value}"
            for r in ctx.restricted_items
        ),
...
class RiskEngine:
    ...
    def check_order(
        ...
        def reject(
            check_name: str, reason: RejectReason, detail: str
        ) -> RiskAssessment:
        ...
        if not ctx.market_status.tradable:
            return reject(
                "market_tradable",
                RejectReason.MARKET_NOT_TRADABLE,
        ...
        for item in ctx.restricted_items:
            ...
            if item.restriction_type is RestrictionType.NO_TRADING:
                return reject(
                    "restricted_list",
                    RejectReason.RESTRICTED_INSTRUMENT,
                    "Restricted List: 거래 전면 금지 종목입니다",
                )
            if (
                item.restriction_type is RestrictionType.NO_NEW_POSITION
                and not reducing
            ):
                return reject(
                    "restricted_list",
        ...
        if ref_price is None:
            return reject(
                "notional_bounds",
                RejectReason.NOTIONAL_BELOW_MINIMUM,
        ...
        projected_issuer_exposure = (
            ctx.portfolio.issuer_exposure.get(issuer, Decimal(0)) + signed_notional
        )
        ...
        if intent.side is Side.BUY and issuer_pct > ctx.limits.hard_single_issuer_pct:
            return reject(
                "concentration",
                RejectReason.CONCENTRATION_LIMIT_HARD,
        ...
        if intent.side is Side.BUY and issuer_pct > ctx.limits.soft_single_issuer_pct:
            current_issuer_exposure = ctx.portfolio.issuer_exposure.get(
                issuer, Decimal(0)
            )
            max_allowed_notional = max(
                Decimal(0),
            ...
            checks.append(
                CheckOutcome(
                    "concentration",
                    True,
                    f"{issuer} Soft Limit로 {allowed_qty}주로 축소",
                )
            )
        else:
        ...
        if order_notional > remaining_turnover:
            ...
            checks.append(
                CheckOutcome(
                    "turnover",
                    True,
                    f"잔여 회전율 한도로 {allowed_qty}주로 축소",
                )
            )
        else:
        ...
        if ctx.trading_state is TradingState.HALTED:
            return reject(
                "trading_state",
                RejectReason.TRADING_STATE_BLOCKED,
                "Trading State가 HALTED입니다 - 모든 주문이 차단됩니다",
            )
        if (
            ctx.trading_state in (TradingState.ENTRY_BLOCKED, TradingState.REDUCE_ONLY)
            and not reducing
        ):
            return reject(
                "trading_state",
        ...
        if not reducing and ctx.portfolio.daily_pnl <= -ctx.limits.max_daily_loss:
            return reject(
                "trading_state",
                RejectReason.LOSS_LIMIT_BREACHED,
        ...
        if ctx.counterparty.health is CounterpartyHealth.DOWN:
            return reject(
                "counterparty_health",
                RejectReason.COUNTERPARTY_UNHEALTHY,
        ...
        checks.append(
            CheckOutcome(
                ...
                True,
                ""
                if ctx.counterparty.health is CounterpartyHealth.OK
                else "DEGRADED 상태 - 통과하되 기록",
            )
        )
        ...
        verdict = (
            RiskVerdict.APPROVE
            if allowed_qty == intent.quantity
            else RiskVerdict.RESIZE
        )
        decision = RiskDecision(
            order_intent_id=intent.order_intent_id,
            verdict=verdict,
...
if __name__ == "__main__":
    ...
    sys.path.insert(
        0, str(Path(__file__).resolve().parent.parent.parent / "02-trading" / "oms")
    )
    from oms import OMS
    ...
    def base_context(**overrides) -> RiskContext:
        defaults = {
            ...
            "counterparty": CounterpartyStatus(
                broker_adapter="paper", health=CounterpartyHealth.OK
            ),
    ...
    def approved(fn, why: str):
        result = fn()
        assert result.decision.verdict is RiskVerdict.APPROVE, (
            f"승인됐어야 함: {why} ({result.decision.reason})"
        )
        return result

    def resized(fn, why: str):
        result = fn()
        assert result.decision.verdict is RiskVerdict.RESIZE, (
            f"축소됐어야 함: {why} ({result.decision.reason})"
        )
        return result
    ...
    r = approved(
        lambda: engine.check_order(make_intent(), base_context()), "평범한 매수"
    )
    ...
    assert r.approved_legs and r.approved_legs[0]["order_intent_id"] == str(
        r.decision.order_intent_id
    )
    ...
    assert (
        r_repeat.input_hash
        == engine.check_order(same_intent, base_context()).input_hash
    ...
    tight_mandate_ctx = base_context(
        mandate=MandateScope(
            fund_id=fund,
            allowed_instrument_ids=None,
            min_order_notional=Decimal(100000),
            max_order_notional=Decimal(3500000),
        )
    )
    r = resized(
        lambda: engine.check_order(make_intent(), tight_mandate_ctx),
        "최대 Notional 초과",
    )
    assert r.decision.approved_quantity == Decimal(50), (
        "3,500,000 / 70,000 = 50주여야 함"
    )
    assert RejectReason.NOTIONAL_ABOVE_MAXIMUM in r.reason_codes

    # 4. Notional 최소 미달 -> REJECT
    below_min_ctx = base_context(
        mandate=MandateScope(
            fund_id=fund,
            allowed_instrument_ids=None,
            min_order_notional=Decimal(50000000),
            max_order_notional=Decimal(100000000),
        )
    )
    r = rejected(
        lambda: engine.check_order(make_intent(), below_min_ctx), "최소 Notional 미달"
    )
    assert RejectReason.NOTIONAL_BELOW_MINIMUM in r.reason_codes

    # 5. Restricted List(NO_TRADING) -> REJECT
    restricted_ctx = base_context(
        restricted_items=(
            RestrictedItem(aapl, RestrictionType.NO_TRADING, now - timedelta(days=1)),
        )
    )
    r = rejected(
        lambda: engine.check_order(make_intent(), restricted_ctx), "거래 전면 금지 종목"
    )
    ...
    no_new_pos_ctx = base_context(
        restricted_items=(
            RestrictedItem(
                aapl, RestrictionType.NO_NEW_POSITION, now - timedelta(days=1)
            ),
        ),
        portfolio=PortfolioState(
            fund_id=fund,
    ...
    rejected(
        lambda: engine.check_order(make_intent(), no_new_pos_ctx),
        "NO_NEW_POSITION - 매수는 막힘",
    )
    approved(
        lambda: engine.check_order(
            make_intent(side=Side.SELL, qty="50"), no_new_pos_ctx
        ),
        "NO_NEW_POSITION이어도 청산 매도는 통과해야 함",
    )

    # 7. Mandate 유니버스 밖 종목 -> REJECT
    narrow_mandate_ctx = base_context(
        mandate=MandateScope(
            fund_id=fund,
            allowed_instrument_ids=frozenset({msft}),
            min_order_notional=Decimal(100000),
            max_order_notional=Decimal(100000000),
        )
    )
    r = rejected(
        lambda: engine.check_order(make_intent(instrument=aapl), narrow_mandate_ctx),
        "유니버스 밖 종목",
    )
    ...
    r = rejected(
        lambda: engine.check_order(make_intent(snapshot=snap(as_of=now)), stale_ctx),
        "오래된 스냅샷",
    )
    assert RejectReason.STALE_SNAPSHOT in r.reason_codes

    # 9. 시장 거래 불가 -> REJECT
    halted_market_ctx = base_context(
        market_status=MarketStatus(tradable=False, reason="거래정지")
    )
    r = rejected(
        lambda: engine.check_order(make_intent(), halted_market_ctx), "거래정지 종목"
    )
    assert RejectReason.MARKET_NOT_TRADABLE in r.reason_codes

    # 10. 매수 여력 부족 -> RESIZE
    low_buying_power_ctx = base_context(
        portfolio=PortfolioState(
            fund_id=fund,
            cash=Decimal(3500000),
            buying_power=Decimal(3500000),
            gross_exposure=Decimal(100000000),
            peak_equity=Decimal(1000000000),
            equity=Decimal(1000000000),
        )
    ...
    r = resized(
        lambda: engine.check_order(make_intent(), low_buying_power_ctx),
        "매수 여력 부족",
    )
    assert r.decision.approved_quantity == Decimal(50)
    ...
    oversell_ctx = base_context(
        portfolio=PortfolioState(
            fund_id=fund,
            cash=Decimal(100000000),
            buying_power=Decimal(100000000),
            gross_exposure=Decimal(100000000),
            positions={aapl: Decimal(30)},
            peak_equity=Decimal(1000000000),
            equity=Decimal(1000000000),
        )
    )
    r = resized(
        lambda: engine.check_order(
            make_intent(side=Side.SELL, qty="100"), oversell_ctx
        ),
        "보유량 초과 매도",
    )
    ...
    rejected(
        lambda: engine.check_order(
            make_intent(side=Side.SELL, qty="10", instrument=msft), oversell_ctx
        ),
        "미보유 종목 매도는 축소가 아니라 거부",
    )

    # 12. 집중도 Hard Limit 초과 -> REJECT
    conc_hard_ctx = base_context(
        portfolio=PortfolioState(
            fund_id=fund,
            cash=Decimal(100000000),
            buying_power=Decimal(100000000),
    ...
    r = resized(
        lambda: engine.check_order(make_intent(qty="10"), conc_soft_ctx),
        "집중도 Soft Limit 초과",
    )
    assert RejectReason.CONCENTRATION_LIMIT_SOFT in r.reason_codes

    # 14. 일일 회전율 한도 초과 -> RESIZE
    turnover_ctx = base_context(
        portfolio=PortfolioState(
            fund_id=fund,
            cash=Decimal(100000000),
            buying_power=Decimal(100000000),
            gross_exposure=Decimal(100000000),
            notional_traded_today=Decimal(96500000),
            peak_equity=Decimal(1000000000),
            equity=Decimal(1000000000),
        ),
        limits=LimitSet(
    ...
    r = resized(
        lambda: engine.check_order(make_intent(), turnover_ctx), "회전율 한도 초과"
    )
    assert RejectReason.TURNOVER_LIMIT in r.reason_codes

    # 15. 일일 주문 건수 한도 초과 -> REJECT
    order_count_ctx = base_context(
        portfolio=PortfolioState(
            fund_id=fund,
            cash=Decimal(100000000),
            buying_power=Decimal(100000000),
    ...
    r = rejected(
        lambda: engine.check_order(make_intent(), order_count_ctx),
        "일일 주문 건수 한도",
    )
    ...
    rejected(
        lambda: engine.check_order(make_intent(), halted_ctx), "HALTED - 신규 매수"
    )
    rejected(
        lambda: engine.check_order(make_intent(side=Side.SELL, qty="50"), halted_ctx),
        "HALTED - 청산도 막힘",
    )
    ...
    rejected(
        lambda: engine.check_order(make_intent(), entry_blocked_ctx),
        "ENTRY_BLOCKED - 신규 매수 차단",
    )
    approved(
        lambda: engine.check_order(
            make_intent(side=Side.SELL, qty="50"), entry_blocked_ctx
        ),
        "ENTRY_BLOCKED이어도 청산은 통과해야 함",
    )

    # 18. 일일 손실 한도 초과 -> REJECT (신규 진입), 청산은 통과
    loss_limit_ctx = base_context(
        portfolio=PortfolioState(
            fund_id=fund,
            cash=Decimal(100000000),
            buying_power=Decimal(100000000),
    ...
    r = rejected(
        lambda: engine.check_order(make_intent(), loss_limit_ctx), "일일 손실 한도 초과"
    )
    assert RejectReason.LOSS_LIMIT_BREACHED in r.reason_codes
    approved(
        lambda: engine.check_order(
            make_intent(side=Side.SELL, qty="50"), loss_limit_ctx
        ),
        "손실 한도 초과해도 청산은 통과해야 함",
    )

    # 19. Drawdown 한도 초과 -> REJECT
    drawdown_ctx = base_context(
        portfolio=PortfolioState(
            fund_id=fund,
            cash=Decimal(100000000),
            buying_power=Decimal(100000000),
            gross_exposure=Decimal(100000000),
            peak_equity=Decimal(1000000000),
            equity=Decimal(750000000),
        )
    )
    r = rejected(
        lambda: engine.check_order(make_intent(), drawdown_ctx), "Drawdown 한도 초과"
    )
    assert RejectReason.DRAWDOWN_LIMIT_BREACHED in r.reason_codes

    # 20. 브로커 DOWN -> REJECT / DEGRADED -> 통과하되 기록
    down_ctx = base_context(
        counterparty=CounterpartyStatus("paper", CounterpartyHealth.DOWN)
    )
    ...
    degraded_ctx = base_context(
        counterparty=CounterpartyStatus("paper", CounterpartyHealth.DEGRADED)
    )
    approved(
        lambda: engine.check_order(make_intent(), degraded_ctx),
        "DEGRADED는 차단하지 않고 통과",
    )
    ...
    r = rejected(
        lambda: engine.check_order(make_intent(), combo_ctx), "Hard 조건 우선 차단"
    )
    assert r.reason_codes == (RejectReason.MARKET_NOT_TRADABLE,), (
        "market_tradable에서 바로 멈춰야 함"
    )
