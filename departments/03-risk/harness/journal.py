"""Risk 실행 로그·리플레이·리뷰 계약.

Hermes 부서 경계와 LangGraph 직원 실행을 한 run_id로 묶는다. 이 모듈은
주문을 만들거나 승인하지 않으며, 원문 JSONL을 내보낼 수 있는 결정론적
인메모리 원장이다. 운영 DB는 event_sink로 연결하고, 실패 시 Risk 판단을
확대하지 않는 것이 호출자의 책임이다.
"""
...
class RunJournal:
    """Append-only run journal with explicit replay and review operations."""

    def __init__(
        self,
        *,
        department: str = "risk-management",
