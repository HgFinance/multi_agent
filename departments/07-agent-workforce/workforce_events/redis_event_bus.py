#!/usr/bin/env python3
"""F19 improvement-worker: 다른 본부의 Domain Event를 소비하는 Redis Streams adapter.

담당: 영주 (Agent Workforce 인사팀)
근거: docs/02-engineering/DEPARTMENT_BACKEND_INTEGRATION_DOCKER_PLAN.md 3.4절(Redis Streams +
      Transactional Outbox), 8.2절(Event Envelope), 8.3절(Stream 분리 - hf:workforce),
      departments/00-ceo-office/governance_events/redis_event_bus.py와 동일 패턴
      (departments/06-ai-qa-audit/qa_events/redis_event_bus.py가 원형).

이 모듈은 판정 로직이 없다. Redis Stream Publish/Consume 배관만 한다 - QA Eval 결과를
Shadow/Reject으로 전이할지는 improvements/workflow.py의 ImprovementWorkflow가 결정한다.
...
DEFAULT_STREAM = "hf:workforce"
DEFAULT_GROUP = "workforce-improvement-consumers"
DEFAULT_DEDUPE_TTL_SECONDS = 7 * 24 * 60 * 60


class WorkforceEventBusError(RuntimeError):
    """Workforce Event Bus 처리 실패."""
...
class RedisEventBus:
    ...
    governance_events/qa_events/risk_events의 RedisEventBus와 동일 구현이다 - 공용
    패키지로 아직 안 뽑은 이유는 governance_events/redis_event_bus.py의 같은 주석 참고
    (부서별 Image·의존성 독립 소유 원칙, 지금 임의로 새 공유 패키지를 만들지 않는다).
    """
    ...
    def __init__(
        ...
        *,
        ...
        consumer: str = "workforce-api",
        dedupe_prefix: str = "workforce:event-processed:",
    ...
    def ensure_group(self) -> None:
        try:
            ...
            if "BUSYGROUP" not in str(exc):
                raise WorkforceEventBusError(f"Redis Consumer Group 생성 실패: {exc}") from exc
    ...
    def publish(self, *, event_id: UUID, event_type: str, trace_id: UUID, payload: dict[str, Any]) -> str:
        try:
            ...
            raise WorkforceEventBusError(f"Workforce Event 발행 실패: {exc}") from exc
        ...
        *,
        ...
        if not messages:
            try:
                ...
                raise WorkforceEventBusError(f"Redis Event 소비 실패: {exc}") from exc
        ...
        for message_id, fields in messages:
            ...
            if not event_id:
                raise WorkforceEventBusError("Event에 event_id가 없습니다")
    ...
    def _claim_pending(self, *, count: int, min_idle_ms: int) -> list[tuple[Any, dict[Any, Any]]]:
        ...
        try:
            ...
            raise WorkforceEventBusError(f"Redis Pending Event 재확보 실패: {exc}") from exc
