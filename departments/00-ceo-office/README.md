...
현재 Head 런타임은 Hermes Profile `ceo-agent` + `openai-codex/gpt-5.6-luna`이며, 승인된 Claude Code를 대체 런타임으로 사용할 수 있다. CEO의 직원은 독립 LangGraph `executive-briefing-worker` + Ollama `qwen3:8b`다. `Modelfile`의 `hermes3` alias는 로컬·역사적 보조 실행용이며 현재 Head/Worker 기준이 아니다. Build·Eval·권한 기준은 [Ollama Department Modelfile Guide](../../docs/02-engineering/OLLAMA_DEPARTMENT_MODELFILE_GUIDE.md)를 따른다.
