...
그래서 **트레이딩 7명은 전원 `forced_route`** 다 — `choose_rag_route()` 가 payload 를
읽는 분기에 애초에 도달하지 못한다.
...
WORKER_RAG_POLICIES: dict[str, WorkerRAGPolicy] = {
    ...
    "bear-thesis-worker": WorkerRAGPolicy(
        frozenset({"NO_RAG"}), "NO_RAG",
        "Claim 색인 외 검색은 상대가 못 본 근거를 들여와 독립성 검증을 우회한다"),
    # 이번 범위에서 RAG 배정 대상이 아니다. **비워두지 않고 명시적으로 닫는다** -
    # 정책표에 없으면 rag_policy_for_worker 가 예외를 내지만, 나중에 누가 기본값을
    # 열어두는 실수를 하면 이 직원이 조용히 검색을 얻는다.
    "trade-proposal-worker": WorkerRAGPolicy(
        frozenset({"NO_RAG"}), "NO_RAG", "RAG 미배정(범위 밖) - 기본 열림 방지를 위해 명시적으로 닫는다"),
    # 상태 전이표는 간선 집합이라 그래프다. "지금 상태에서 도달 가능한 상태"가 홉 탐색이고,
    # 2홉을 넘기면 계획이 아니라 추측이다.
    "order-constraint-worker": WorkerRAGPolicy(
        frozenset({"GRAPH"}), "GRAPH",
        "INTENT/BROKER 전이표는 간선 집합 - 도달 가능 상태 탐색이 필요하다"),
    # broker_rules.search() 는 TR 코드 정확일치 + 토큰 겹침(lexical),
    # tca_memory.recall() 은 철학·방향·규모·adapter 정형 필터. 벡터도 홉도 없다.
    "execution-planning-worker": WorkerRAGPolicy(
        frozenset({"HYBRID"}), "HYBRID", "브로커 규칙(lexical) + 집행 기억(정형 필터)"),
    "venue-cost-worker": WorkerRAGPolicy(
        frozenset({"HYBRID"}), "HYBRID", "브로커 규칙(lexical) + 집행 기억(정형 필터)"),
    # Certification 은 조회 한 번이지 검색이 아니다. 검색을 붙이면
    # "인증 문서를 찾았다"가 "인증이 있다"로 새기 쉽다.
    "derivatives-structure-worker": WorkerRAGPolicy(
        frozenset({"NO_RAG"}), "NO_RAG",
        "Certification 은 서명 조회다 - 검색을 붙이면 문서 발견이 인증 보유로 오독된다"),
}
...
def choose_rag_route(payload: Mapping[str, Any], *, worker_id: str) -> RAGPlan:
    ...
    트레이딩 7명은 전원 forced 라 지금은 아래 payload 분기에 도달하지 않는다 -
    그게 의도다. 자체 점검이 그 사실을 직접 검사한다.
    """
...
if __name__ == "__main__":
    ...
    TRADING_WORKERS = (
        "bull-thesis-worker", "bear-thesis-worker", "trade-proposal-worker",
        "order-constraint-worker", "execution-planning-worker",
        "venue-cost-worker", "derivatives-structure-worker",
    )

    # 1. 직원 7명 전원 정책이 있다. 모르는 직원은 기본값을 못 얻는다
    assert set(WORKER_RAG_POLICIES) == set(TRADING_WORKERS), sorted(WORKER_RAG_POLICIES)
    for worker_id in TRADING_WORKERS:
    ...
    assert choose_rag_route(attack, worker_id="venue-cost-worker").route == "HYBRID"
    ...
    hybrid = choose_rag_route({}, worker_id="execution-planning-worker")
    assert "lexical" in hybrid.methods and "vector" not in hybrid.methods, hybrid.methods
    assert hybrid.max_hops == 0, "정형 검색에 홉이 붙었다"
    graph = choose_rag_route({}, worker_id="order-constraint-worker")
    assert graph.route == "GRAPH" and graph.max_hops == 2, graph
    assert set(no_rag.as_dict()) == {"route", "methods", "max_chunks", "max_hops", "reason"}
    print("  경로별 플랜 모양             OK")

    # 5. provider 강제 헬퍼 - 라우터를 장식이 아니게 만드는 지점
    ...
    # 6. Bull 과 Bear 는 같은 정책이어야 한다 - 한쪽만 검색을 얻으면 그게 편향이다
    ...
    print("ok - 트레이딩 RAG 라우터 6개 영역 점검 통과 "
          f"(직원 {len(TRADING_WORKERS)}명 전원 forced, 입력으로 경로 변경 불가)")
