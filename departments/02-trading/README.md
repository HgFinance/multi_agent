...
Local Ollama Alias는 [`Modelfile`](Modelfile)의 `qwen2.5-coder` 기반 `agent-trading`이고 Hermes Profile은 `trading-department`다. Build·Eval·권한 기준은 [Ollama Department Modelfile Guide](../../docs/02-engineering/OLLAMA_DEPARTMENT_MODELFILE_GUIDE.md)를 따른다.
현재 실행 상태와 도현님 Daily Scrum은 [실행 현황과 통합 계획 v2.1](../../docs/PROJECT_IMPLEMENTATION_STATUS.md#42-도현님-트레이딩본부-회계포트폴리오본부와-공통-platform)을 따른다.
