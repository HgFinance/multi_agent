...
class ExecutionAuthority(StrEnum):
    """이 주문이 **왜 존재해도 되는가**. 없으면 전송하지 않는다.

    Risk 승인은 "한도 안인가" 를 답하지 자본을 걸어도 되는지는 답하지 않는다.
    자본이 걸린 긍정 판정에는 사람 서명이 필요한데, **서명의 대상이 둘로 갈린다.**

      STRATEGY  자동 실행. 사람은 **전략 승격 때 한 번** 서명했다. 매 주문마다
                서명받으면 자동매매가 성립하지 않는다. 대신 승격에 한도·만료가
                붙고, 승격이 철회되면 그 순간부터 이 권한도 없다.
      USER      승인 실행. 사람이 **이 주문 한 건**에 서명했다. 전략이 아니라
                일회성 제안(포트폴리오 자문 등)이라 재사용되지 않는다.

    둘 다 `authority_ref` 로 **어느 승격/승인인지**를 가리킨다. 참조가 없으면
    "누가 허락했나" 를 나중에 물을 수 없고, 그건 서명이 없는 것과 같다.
    """
